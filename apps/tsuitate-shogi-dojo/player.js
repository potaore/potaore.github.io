...
(function() {
  var Board, BordCanvas, Game, KifuPlayer, Koma, KomaType, KomadaiCanvas, MoveConvertor, Node, Socket, SyogiJudgement, computeDuration, currentRoom, domFinder, game, getKomaName, getimage, images2, judgement, km1, km2, komaTypes, listenlist, moveConvertor, node, util,
  ...
  getKomaName = function(id) {
    if (komaTypes[id].resourceId) {
      return komaTypes[komaTypes[id].resourceId].name;
    } else {
      return komaTypes[id].name;
    }
  };
  ...
  SyogiJudgement = (function() {
    ...
    SyogiJudgement.prototype.canMove = function(game, playerNumber, moveInfo) {
      var fromKoma, n, pos, toKoma, toKomaType;
      n = 0;
      ...
      if (fromKoma.komaType !== moveInfo.from.komaType) {
        return false;
      }
      if (toKomaType.id !== fromKoma.komaType && toKomaType.resourceId !== fromKoma.komaType) {
        return false;
      }
      ...
      pos = _(moveConvertor.getMovablePos(playerNumber, moveInfo.from.komaType, moveInfo.from.position, game.board)).find(function(pos) {
  ...
  KifuPlayer = (function() {
    ...
    KifuPlayer.prototype.readTe = function(te) {
      ...
      if (te.info.type === "moveKoma") {
        ...
        currentBoard.events.push({
          "type": "endGame",
          "winPlayerNumber": te.info.winPlayerNumber
        });
      }
  ...
  node.connect({
    ...
    var bordCanvas, clearKoma, drawKoma, flipBord, init, initializeTableState, komadai1Canvas, komadai2Canvas, memo, params, redrawKoma, reredrawKoma, resetGameInfo, resetPlayerInfo, setPlayerInfo, showMessage, showMovableCell, showNariDiv, showPutableCell, updateGameInfo;
    ...
    graphicApiSocket.on("graphicApi.init", function(command) {
      init();
      return flipBord(false);
    });
    init();
    updateGameInfo = function(command) {
    ...
    resetGameInfo = function() {
      if (memo.gameInfo) {
        return updateGameInfo(memo.gameInfo);
      }
    };
    graphicApiSocket.on("graphicApi.updateGameInfo", function(command) {
      return updateGameInfo(command);
    });
    setPlayerInfo = function(command) {
    ...
    resetPlayerInfo = function() {
      if (memo.playerInfo) {
        return setPlayerInfo(memo.playerInfo);
      }
    };
    graphicApiSocket.on("graphicApi.setPlayerInfo", function(command) {
      return setPlayerInfo(command);
    });
    ...
    reredrawKoma = function() {
      if (memo.game) {
        return redrawKoma(memo.game);
      }
    };
    drawKoma = function(arg) {
      var blind, cancel, komadai1, komadai2, myTurn;
      blind = arg.board.endGame ? 0 : params.blind;
      if (graphicApi.invatedPosition) {
        cancel = false;
        if (blind) {
          myTurn = (arg.board.turn % 2 + 1) === params.blind;
          if (!myTurn && _(arg.board.events).every(function(ev) {
            return ev.type !== "get";
          })) {
            cancel = true;
          }
    ...
    graphicApiSocket.on("graphicApi.showEvents", function(board) {
      var end, foul, myTurn, texts;
      texts = [];
      if (params.blind !== 0) {
        myTurn = (board.turn % 2 + 1) === params.blind;
        foul = false;
        end = false;
        _(board.events).each(function(ev) {
          if (ev.type === "get") {
            if (myTurn) {
              return texts.push(getKomaName(ev.komaType) + "を取りました");
            } else {
              return texts.push(komaTypes[ev.komaType].name + "を取られました");
            }
          } else if (ev.type === "oute") {
            if (myTurn) {
              return texts.push("王手をかけました");
            } else {
              return texts.push("王手をかけられました");
            }
          } else if (ev.type === "foul") {
            foul = true;
            if (myTurn) {
              return texts.push("反則です");
            } else {
              return texts.push("相手が反則手を指しました");
            }
          } else if (ev.type === "endGame") {
            end = true;
            if (ev.winPlayerNumber === 3 - params.blind) {
              return texts.push("あなたの勝ちです");
            } else {
              return texts.push("あなたの負けです");
            }
          }
        });
        if (!end) {
    ...
    showMessage = function(message) {
      ...
      _(message.texts).each(function(text) {
        return $("#gameMessageArea").append($('<li>').text(text));
      });
      $("#gameMessageArea").hide();
      return $("#gameMessageArea").show(200);
    };
    ...
    graphicApiSocket.on("graphicApi.blindKoma", function(arg) {
      params.blind = arg.blind;
      flipBord(arg.flip);
      resetPlayerInfo();
      return resetGameInfo();
    });
    flipBord = function(flip) {
      if (flip) {
        $("#boardImg").css({
  ...
  node.connect({
    ...
    kifuApiSocket.on("kifuApi.replayKifu", function(arg) {
      ...
      refresh = function(index, board) {
        ...
        return node.emit("graphicApi.showEvents", board);
      };
  ...
  node.connect({
    ...
    soundApiSocket.on("soundApi.playOuteSound", function() {
      return playSound("oute");
    });
    soundApiSocket.on("soundApi.playGetSound", function() {
